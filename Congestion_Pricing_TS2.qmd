...
As of September 2026, an MVP of the dashboard is live on Streamlit, forecasting raw traffic volumes
in a 2-week period following the most recent data pulls.

[Check it out!](https://crz-traffic-map.streamlit.app/)

# Introduction

On January 5, 2025, New York City's [Congestion Pricing Program](https://www.nytimes.com/2025/01/04/nyregion/congestion-pricing-nyc.html) went into effect, where vehicles would be tolled for entering Manhattan's *Congestion Relief Zone* (**CRZ**). This toll is \$9 during peak period (5AM to 9PM on weekdays) and \$2.25 otherwise. The debates over congestion pricing, its enacting and its merits are still ongoing today, the fact remains it's currently in place, with plans to gradually increase the tolls over the next 6 years until it caps out at \$15 in 2031.[^mta]:
...
For this project, I'll be analyzing and modeling `Traffic Volume` in the *Central Business District* or **CBD** since *Congestion Pricing* took effect.

Using a Neural-Network model, *Long Short-Term Memory* or **LSTM**, I will model short-windows of traffic volume to try and predict what traffic in the **CBD** will look like in the near-future, coming from the `Detection Region` of `Brooklyn`.

If the model is trained well enough, I will deploy it to a *Streamlit* dashboard to order to have live predictions about traffic data in the near-future from whatever date the dashboard is accessed.

This project's stretch goal will also be able to model all the different `Detection Group` on this deployable dashboard, so users can ask questions such as "if I want to enter Manhattan at 7am on December 20th from Brooklyn, is it better to by using the Brooklyn Bridge, the Manhattan Bridge, the Williamsburg Bridge, etc.?"
...
I've staged a dashboard using *Streamlit* to try and give predictions for the [upcoming week of traffic data](https://sethkauf-congestion-traffic-dashboard.streamlit.app/).

It's still very much a work-in-progress, but maybe it can help on someone's morning commute.
...
# Update 2026-July
I've been working through this dashboard to make an interactive Streamlit map with live `MCP` calls on the data to provide up-to-the-hour predictions

# Update 2026-September
An MVP of the Dashboard is up with all 12 regions predicted traffic forecasts in a 2-week period following the most recent data-read, check it out!

https://crz-traffic-map.streamlit.app/